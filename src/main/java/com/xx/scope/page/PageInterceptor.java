...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		Object parObj = boundSql.getParameterObject();
		// 通过反射获取delegate父类BaseStatementHandler的mappedStatement属性
		if(parObj instanceof Page<?>){
			Page<?> page = (Page<?>) parObj;
			MappedStatement mappedStatement = (MappedStatement) ReflectUtil.getFieldValue(delegate, "mappedStatement");
			//根据方法名称来判断是不是需要分页查询
			if(mappedStatement.getId().lastIndexOf("selectPageInfo") >= -1){
	...
	private String getMysqlPageSql(StringBuffer sqlBuffer) {
		...
		int offset = (SystemContext.getOffset()) * SystemContext.getPagesize();
		sqlBuffer.append(" limit ").append(offset).append(",").append(SystemContext.getPagesize());
		return sqlBuffer.toString();
	}
	...
	private String getOraclePageSql(StringBuffer sqlBuffer) {
		...
		int offset = (SystemContext.getOffset() - 1) * SystemContext.getPagesize() + 1;
		sqlBuffer.insert(0, "select u.*, rownum r from (").append(") u where rownum < ")
				.append(offset + SystemContext.getPagesize());
	...
	private String getCountSql(String sql) {
		return "select count(1) as totalRecord from (" + sql + ") as t_totalRecord";
	}
